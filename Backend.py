    ...
    for j in range(10):
        while hex_string:
            ...
            while line:
                ...
                if two_bytes:
                    Data1 += two_bytes + ' '
            Data1 =Data1[:-1] +"   " +DATA[i:i+16]+'\n'
            i += 16

    if hex_string:
        for j in range(10):
            while hex_string:
                Data2 += '{:04x}: '.format(index)
                index += 16
                line, hex_string = hex_string[:64], hex_string[64:]
                while line:
                    two_bytes, line = line[2:4], line[4:]
                    if two_bytes:
                        Data2 += two_bytes + ' '
                Data2 =Data2[:-1] +"   " +DATA[i:i+16]+'\n'
                i += 16
                
    if hex_string:    
         for j in range(10):
             while hex_string:
                Data3 += '{:04x}: '.format(index)
                index += 16
                line, hex_string = hex_string[:64], hex_string[64:]
                while line:
                    two_bytes, line = line[2:4], line[4:]
                    if two_bytes:
                        Data3 += two_bytes + ' '
                Data3 =Data3[:-1] +"   " +DATA[i:i+16]+'\n'
                i += 16
                
    if hex_string:   
         for j in range(10):
             while hex_string:
                Data4 += '{:04x}: '.format(index)
                index += 16
                line, hex_string = hex_string[:64], hex_string[64:]
                while line:
                    two_bytes, line = line[2:4], line[4:]
                    if two_bytes:
                        Data4 += two_bytes + ' '
                Data4 =Data4[:-1] +"   " +DATA[i:i+16]+'\n'
                i += 16
                
    if hex_string:
        for j in range(10):
             while hex_string:
                Data5 += '{:04x}: '.format(index)
                index += 16
                line, hex_string = hex_string[:64], hex_string[64:]
                while line:
                    two_bytes, line = line[2:4], line[4:]
